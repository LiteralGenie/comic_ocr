# adapted from https://github.com/mindee/doctr/blob/707ca424e794cff8a065369883d7a678c0898e06/references/recognition/train_pytorch.py
...
@torch.no_grad()
def evaluate(args, model, val_loader, batch_transforms, val_metric, amp=False):
    # Model in eval mode
    model.eval()
    # Reset val metric
    val_metric.reset()
...
def train_recognition(args):
    ...
    for epoch in range(args.epochs):
        fit_one_epoch(
            model, train_loader, batch_transforms, optimizer, scheduler, amp=args.amp
        )
