...
def main():
    ...
    labels = {
        ...
        for r in db.execute("SELECT id, data FROM detection_labels")
    }
    for k in labels:
    ...
    (DATASET_DIR / "detection" / "train_labels.json").write_text(
        json.dumps(train_labels),
    )
    (DATASET_DIR / "detection" / "val_labels.json").write_text(
        json.dumps(val_labels),
    )

    train_detection(
        Namespace(
            dataset_path=str(DATASET_DIR / "detection"),
